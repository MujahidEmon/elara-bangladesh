    ...
    <header className=" bg-white shadow-sm">
      ...
      >
        <div className="mx-auto flex h-10 justify-center max-w-7xl items-center gap-7 overflow-x-auto px-4">
          <Link
            href={'/'}
            className={`inline-flex shrink-0 items-center gap-1.5 text-base  text-white transition hover:text-black ${pathname === '/' ? "text-blue-600" : ""
              }`}
          >Home
          </Link>
          <Link
          ...
          {visibleCategories.map((category) => (
            <Link
              key={category.name}
              href={category.href}
              className={`inline-flex shrink-0 items-center gap-1.5 text-base transition ${isActive(category.href)
            ...
            >
              ...
              {category.hasDropdown && (
                <FiChevronDown size={17} strokeWidth={2.2} />
              )}
            </Link>
          ))}
        </div>
